...
var _ = Describe("ScalityUIComponentExposer Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should update deployed UI apps ConfigMap when it exists", func() {
			By("Creating deployed UI apps ConfigMap in operator namespace")
			...
			defer func() { _ = k8sClient.Delete(ctx, deployedAppsConfigMap) }()

			By("Creating component with status fields")
			component := &uiv1alpha1.ScalityUIComponent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-component-deployed",
					Namespace: testNamespace,
				},
				Spec: uiv1alpha1.ScalityUIComponentSpec{
					Image: "scality/component:latest",
			...
			exposer := &uiv1alpha1.ScalityUIComponentExposer{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-exposer-deployed",
					Namespace: testNamespace,
				},
				Spec: uiv1alpha1.ScalityUIComponentExposerSpec{
					ScalityUI:          uiName,
					ScalityUIComponent: "test-component-deployed",
					AppHistoryBasePath: "/app/monitoring",
				},
			}
			Expect(k8sClient.Create(ctx, exposer)).To(Succeed())
			defer func() { _ = k8sClient.Delete(ctx, exposer) }()
			...
			By("Verifying deployed UI apps ConfigMap was updated")
			...
			Eventually(func() bool {
				...
				if len(deployedApps) != 1 {
					return false
				}

				app := deployedApps[0]
				return app.Name == "test-component-deployed" &&
					app.Kind == "monitoring" &&
					app.URL == "/monitoring" &&
					app.Version == "1.2.3" &&
					app.AppHistoryBasePath == "/app/monitoring"
			}, time.Second*10, time.Millisecond*250).Should(BeTrue())

			By("Checking status condition")
			updatedExposer := &uiv1alpha1.ScalityUIComponentExposer{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name:      "test-exposer-deployed",
				Namespace: testNamespace,
			...
			deployedAppsCondition := meta.FindStatusCondition(updatedExposer.Status.Conditions, "DeployedAppsReady")
			Expect(deployedAppsCondition).NotTo(BeNil())
			Expect(deployedAppsCondition.Status).To(Equal(metav1.ConditionTrue))
		})

		It("should handle missing deployed UI apps ConfigMap gracefully", func() {
			By("Creating exposer without deployed UI apps ConfigMap")
			exposer := &uiv1alpha1.ScalityUIComponentExposer{
				ObjectMeta: metav1.ObjectMeta{
					Name:      exposerName,
					Namespace: testNamespace,
			...
			By("Checking status condition shows success")
			updatedExposer := &uiv1alpha1.ScalityUIComponentExposer{}
			Expect(k8sClient.Get(ctx, typeNamespacedName, updatedExposer)).To(Succeed())
		...
		It("should update existing component entry in deployed UI apps", func() {
			By("Creating deployed UI apps ConfigMap with existing entry")
			operatorNamespace := "scality-ui" // Use fixed namespace for tests
			existingApps := []DeployedUIApp{
				{
					Name:               "existing-component",
					Kind:               "dashboard",
					URL:                "/dashboard",
					Version:            "1.0.0",
					AppHistoryBasePath: "/app/dashboard",
				},
				{
					Name:               componentName,
					Kind:               "old-kind",
					URL:                "/old-url",
					Version:            "0.0.1",
					AppHistoryBasePath: "/old-path",
				},
			}
			existingAppsJSON, _ := json.Marshal(existingApps)
			...
			deployedAppsConfigMap := &corev1.ConfigMap{
				...
				Data: map[string]string{
					"deployed-ui-apps.json": string(existingAppsJSON),
				},
			}
			Expect(k8sClient.Create(ctx, deployedAppsConfigMap)).To(Succeed())
			defer func() { _ = k8sClient.Delete(ctx, deployedAppsConfigMap) }()

			By("Updating component status")
			component := &uiv1alpha1.ScalityUIComponent{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name:      componentName,
				Namespace: testNamespace,
			}, component)).To(Succeed())
			component.Status.Kind = "new-kind"
			component.Status.PublicPath = "/new-url"
			component.Status.Version = "2.0.0"
			Expect(k8sClient.Status().Update(ctx, component)).To(Succeed())
			...
			By("Verifying component entry was updated")
			updatedConfigMap := &corev1.ConfigMap{}
			Eventually(func() bool {
				err := k8sClient.Get(ctx, types.NamespacedName{
					Name:      uiName + "-deployed-ui-apps",
				...
				var deployedApps []DeployedUIApp
				err = json.Unmarshal([]byte(updatedConfigMap.Data["deployed-ui-apps.json"]), &deployedApps)
				if err != nil || len(deployedApps) != 2 {
					return false
				}

				// Check existing component is preserved
				existingFound := false
				for _, app := range deployedApps {
					if app.Name == "existing-component" {
						existingFound = app.Kind == "dashboard" && app.URL == "/dashboard"
						break
					}
				}

				// Check our component was updated
				updatedFound := false
				for _, app := range deployedApps {
					if app.Name == componentName {
						updatedFound = app.Kind == "new-kind" &&
							app.URL == "/new-url" &&
							app.Version == "2.0.0" &&
							app.AppHistoryBasePath == "/new-path"
						break
					}
				}

				return existingFound && updatedFound
			}, time.Second*10, time.Millisecond*250).Should(BeTrue())
		})
		...
		It("should trigger reconciliation when ScalityUIComponent status changes", func() {
			By("Testing findExposersForScalityUIComponent function")
			controllerReconciler := &ScalityUIComponentExposerReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
			}
			...
			By("Finding exposers for component")
			component := &uiv1alpha1.ScalityUIComponent{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name:      componentName,
				Namespace: testNamespace,
			...
			requests := controllerReconciler.findExposersForScalityUIComponent(ctx, component)
			Expect(requests).To(HaveLen(1))
			Expect(requests[0].Name).To(Equal(exposerName))
			Expect(requests[0].Namespace).To(Equal(testNamespace))
		})

		It("should handle empty component status fields", func() {
			By("Creating deployed UI apps ConfigMap")
			operatorNamespace := "scality-ui" // Use fixed namespace for tests
			deployedAppsConfigMap := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      uiName + "-deployed-ui-apps",
			...
			By("Creating component with empty status")
			component := &uiv1alpha1.ScalityUIComponent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-component-empty",
					Namespace: testNamespace,
				},
				Spec: uiv1alpha1.ScalityUIComponentSpec{
					Image: "scality/component:latest",
			...
			exposer := &uiv1alpha1.ScalityUIComponentExposer{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-exposer-empty",
					Namespace: testNamespace,
				},
				Spec: uiv1alpha1.ScalityUIComponentExposerSpec{
					ScalityUI:          uiName,
					ScalityUIComponent: "test-component-empty",
					AppHistoryBasePath: "/app/empty",
				},
			}
			Expect(k8sClient.Create(ctx, exposer)).To(Succeed())
			defer func() { _ = k8sClient.Delete(ctx, exposer) }()
			...
			By("Verifying deployed UI apps entry has empty fields")
			updatedConfigMap := &corev1.ConfigMap{}
			Eventually(func() bool {
				err := k8sClient.Get(ctx, types.NamespacedName{
					Name:      uiName + "-deployed-ui-apps",
				...
				var deployedApps []DeployedUIApp
				err = json.Unmarshal([]byte(updatedConfigMap.Data["deployed-ui-apps.json"]), &deployedApps)
				if err != nil || len(deployedApps) != 1 {
					return false
				}

				app := deployedApps[0]
				return app.Name == "test-component-empty" &&
					app.Kind == "" &&
					app.URL == "" &&
					app.Version == "" &&
					app.AppHistoryBasePath == "/app/empty"
			}, time.Second*10, time.Millisecond*250).Should(BeTrue())
		})
	})
})
